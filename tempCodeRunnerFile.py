...
#14. ST.FORM()
    ...
    # Null aur empty IDs ko filter karo
    valid_ids = df['id'].dropna() if not df.empty else pd.Series()  
    valid_ids = valid_ids[valid_ids.astype(str).str.strip() != '']  
    selected_id = st.selectbox("Select ID", valid_ids.tolist(), key="select_emp")
